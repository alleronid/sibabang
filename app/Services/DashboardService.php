...
class DashboardService{
    ...
    public function getSummaryData($params){
        $now = Carbon::now();
        $lastMonthStart = $now->subMonth()->startOfMonth(); // First day of last month
        $lastMonthEnd = $now->endOfMonth(); // Last day of last month
        $lastweek = $now->subDays(7);

        $revenues = TrxPayment::query()
                ->selectRaw("
                    SUM(CASE WHEN status = 'PAID' THEN total_amount ELSE 0 END) AS total_revenue,
                    SUM(CASE WHEN status = 'PAID' and payed_at >= '$lastweek' THEN total_amount ELSE 0 END) AS last_week_revenue,
                    count(id) AS count_transactions,
                    count(CASE WHEN created_at >= '$lastweek' THEN id ELSE null END) AS last_week_transactions,
                    COUNT(CASE WHEN status = 'PAID' THEN id ELSE NULL END) AS count_success_transactions,
                    count(CASE WHEN status = 'PAID' and created_at >= '$lastweek' THEN id ELSE 0 END) AS last_week_success_transactions,
                    COUNT(CASE WHEN status in ('EXPIRED', 'CANCEL', 'PAID') THEN id ELSE NULL END) AS count_all_transactions,
                    count(CASE WHEN status in ('EXPIRED', 'CANCEL', 'PAID') and created_at >= '$lastweek' THEN id ELSE 0 END) AS last_week_all_transactions
                ")->first();
